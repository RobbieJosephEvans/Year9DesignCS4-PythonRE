...
def onclick(btnInfo):
		...
		B1 = ttk.Button(popup, text = "How we got this definition", command = popup.destoy)
		B1.pack()
		popup.mainloop()

btnInfo = tk.Button(random, text = "Info") #, command = btnInfo.onclick)

btnInfo.grid(pady = (0,10), row = 4, column = 1, columnspan = 1, sticky = "NESW", command = btnInfo)
